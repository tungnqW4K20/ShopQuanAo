...
const Header = () => {
  ...
  const activeClassName = "text-blue-600 font-bold"; // Style for active NavLink

  return (
   
    <>
      <header className="w-full sticky top-0 z-30 bg-white shadow-sm"> {/* Added subtle shadow, adjusted z-index */}
        {/* Top Bar */}
          ...
          <div className="container mx-auto px-4 py-1.5 flex justify-between items-center">
            ...
            <div className="flex items-center space-x-3 sm:space-x-5">
               <Link to="/coolclub" className="flex items-center hover:text-gray-300 transition-colors">
                CoolClub <FiStar className="ml-1 w-3 h-3 sm:w-4 sm:h-4 text-yellow-400" /> {/* Added color */}
              </Link>
              ...
              <button
                ...
                className="hover:text-gray-300 transition-colors focus:outline-none focus:text-gray-100" // Added focus style
              >
                Đăng nhập
              </button>
        ...
        <div className="bg-white border-b border-gray-200">
          <div className="container mx-auto px-4 py-3 sm:py-4 flex justify-between items-center">
            <div className="flex items-center">
              <Link to="/" className="flex items-center space-x-1 text-black focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 rounded-sm"> {/* Added focus */}
            ...
            <nav className="hidden lg:flex items-center space-x-6 xl:space-x-8 font-semibold text-sm text-gray-700">
              <NavLink
                to="/nam"
                className={({ isActive }) =>
                  `px-1 py-0.5 rounded focus:outline-none focus:ring-1 focus:ring-blue-400 hover:text-blue-600 transition-colors ${isActive ? activeClassName : ""}`
                }
              >
                NAM
              </NavLink>
            ...
            <div className="flex items-center space-x-3 sm:space-x-4">
              <div className="relative hidden md:block">
                <label htmlFor="desktop-search" className="sr-only">Tìm kiếm sản phẩm</label> {/* Accessibility */}
                <input
                  id="desktop-search"
                  type="search"
              ...
              <button
                ...
                className="lg:hidden text-gray-600 hover:text-blue-600 focus:outline-none p-1 rounded-md focus:ring-1 focus:ring-blue-500"
        ...
        {isMobileMenuOpen && (
          <div
            id="mobile-menu" 
            className="lg:hidden bg-white border-b border-gray-200 absolute top-full left-0 w-full z-40 shadow-md transition-transform duration-300 ease-in-out origin-top" // Use absolute positioned below header
          >
           <div className="container mx-auto px-4 py-4">
               <div className="relative mb-4 md:hidden">
                <label htmlFor="mobile-search" className="sr-only">Tìm kiếm sản phẩm</label> {/* Accessibility */}
                <input
                  id="mobile-search"
                  type="search"
                  placeholder="Tìm kiếm sản phẩm..."
             ...
             <nav className="flex flex-col space-y-1 font-semibold text-gray-700"> {/* Reduced space-y */}
               ...
               <button
                  type="button"
                  onClick={openLoginModal} 
                  className="px-2 py-1.5 rounded hover:bg-gray-100 hover:text-blue-600 transition-colors block w-full text-left font-semibold text-gray-700" // Ensure styles match other links
                >
                  Đăng nhập / Đăng ký
                </button>
